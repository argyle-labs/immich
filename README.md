...
Self-hosted photo/video (multi-service) — a first-party [orca](https://github.com/argyle-labs/orca) **service-backend**
...
`deploy`, `backup`, `restore`, `configure`, `status`. Rich, immich-specific data is
...
The plugin automates immich, but this repo is self-contained: the docs below (migrated + anonymized from a homelab runbook) let you deploy, configure, and operate it **entirely by hand** on any supported runtime.
